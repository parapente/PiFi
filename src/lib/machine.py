...
class ZMachine:
        ...
        if self.cpu.intr == 1:  # Input interrupt to read a line of text
            if (len(self.cpu.intr_data) > 2):
                self.input.read_line(
                    self.mem.mem[self.cpu.intr_data[0]], self.get_text, self.cpu.intr_data[2], self.intr_line_routine)
            else:
                self.input.read_line(
                    self.mem.mem[self.cpu.intr_data[0]], self.get_text, 0, self.intr_line_routine)
            ...
            self.lex(self.cpu.intr_data[0], self.cpu.intr_data[1],
                     self.cpu.intr_data[2], self.cpu.intr_data[3])
            self.cpu.intr = 0
            self.cpu.start()
            self.handle_intr()
            ...
            if (self.cpu.last_return == 1):  # Routine returned true, we must bail out
                ...
                self.input.read_line(
                    self.mem.mem[self.cpu.intr_data[0]], self.get_text, self.cpu.intr_data[2], self.intr_line_routine, False)
        ...
        while pos < savesize:
            ...
            if nextID == 'CMem':
                ...
                while chunk_pos < (pos + chunk_length):
                    ...
                    chunk_pos += 3
                    eval_stack_size = (
                        filebuf[chunk_pos] << 8) + filebuf[chunk_pos+1]
                    chunk_pos += 2
    ...
    def do_save_state(self):
        ...
        ifhd += [(self.cpu.pc >> 16) & 255, (self.cpu.pc >> 8)
                 & 255, self.cpu.pc & 255, 0]
        tmp = array('B')
        tmp.fromlist(ifhd)
        ...
        tmp = array('B')
        tmp.fromlist([(savefile_size >> 24) & 255, (savefile_size >> 16)
                     & 255, (savefile_size >> 8) & 255, savefile_size & 255])
        self.savefile.write(tmp)
