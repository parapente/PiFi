...
class ZCpu:
    ...
    def _test_attr(self):
        ...
        if self.zver < 4:
            b = (self.mem[obj] << 24) + (self.mem[obj + 1] << 16) + \
                (self.mem[obj + 2] << 8) + self.mem[obj + 3]
            # print "b:",format(b,"X")
            mask = 1 << (31 - ops[1])
    ...
    def _get_prop(self):
        ...
        if prop == 0:  # No defined property, try default
            ...
            data = (self.mem[base + ((ops[1] - 1) * 2)] << 8) + \
                self.mem[base + ((ops[1] - 1) * 2) + 1]
        else:  # Property found
            if self.zver < 4:
                size = self.mem[prop]
                nob = ((size - ops[1]) // 32) + 1
    ...
    def _inc2(self, var):
        if var == 0:
            ...
            self.mem[self.header.global_table +
                     (var - 16) * 2 + 1] = tmp & 0xff
        ...
        for i in range(4):
            ...
            if optype == 0:  # Large constant (2 bytes)
                ...
                if self.mem[self.pc] == 0:
                    ...
                    b1 = self.mem[self.header.global_table +
                                  (self.mem[self.pc] - 16) * 2]
                    b2 = self.mem[self.header.global_table +
                                  (self.mem[self.pc] - 16) * 2 + 1]
    ...
    def _show_status2(self):
        if self.zver < 4:
            ...
            objnum = (self.mem[self.header.global_table] <<
                      8) + self.mem[self.header.global_table + 1]
            ...
            if self.zver < 3:
                ...
                if self.header.score_game() == 0:
                    score_game = True
            if score_game:
                score = (self.mem[self.header.global_table + 2]
                         << 8) + self.mem[self.header.global_table + 3]
                turns = (self.mem[self.header.global_table + 4]
                         << 8) + self.mem[self.header.global_table + 5]
                self.output.print_status(
                    text, "Score: {0} Turns: {1}".format(score, turns))
            else:
                hour = (self.mem[self.header.global_table + 2]
                        << 8) + self.mem[self.header.global_table + 3]
                mins = (self.mem[self.header.global_table + 4]
                        << 8) + self.mem[self.header.global_table + 5]
                if mins < 10:
                    self.output.print_status(
                        text, "{0}:0{1}".format(hour, mins))
                else:
                    self.output.print_status(
                        text, "{0}:{1}".format(hour, mins))
        ...
        if stack.local_vars_num > 0:
            ...
            if self.zver < 5:
                ...
                while lenargv < stack.local_vars_num:
                    stack.local_vars[lenargv] = (
                        mem[self.pc] << 8) + mem[self.pc + 1]
                    lenargv += 1
                    self.pc += 2
    ...
    def branch(self, condition):
        if (self.mem[self.pc] & 64) == 64:  # Offset is 1 byte long
            ...
            if self.mem[self.pc] & 32 == 32:  # Negative offset
                offset = (((self.mem[self.pc] | 0xc0) <<
                          8) + self.mem[self.pc + 1] - 65536)
            else:
                offset = ((self.mem[self.pc] & 63) << 8) + \
                    self.mem[self.pc + 1]
            gf = 2
        jif = 'True'
