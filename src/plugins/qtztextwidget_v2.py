...
class ZTextWidget(QWidget):
    ...
    def __init__(self, parent=None, flags=Qt.Widget):
        ...
        print(self.font_metrics.averageCharWidth(),
              self.linesize, self.avgwidth)
        print(self.font_metrics.height())
        self.width = (self.pbuffer[0].width() -
                      4) // self.font_metrics.averageCharWidth()
    ...
    def scroll(self, painter):
        part = self.pbuffer[0].copy(0, self.linesize, self.pbuffer[0].width(
    ...
    def draw_input_buffer(self):
        ...
        self.lastwindow.set_cursor_position(
            self.insert_pos[0], self.insert_pos[1])
    ...
    def draw_text(self, txt, txtlen, window):
        ...
        if ((txtlen > 0) and not ((txt == self.cursor_char) and (self._cursor_visible == False))):
            ...
            if (txt == '\n'):
                if (window.cursor[1] == self.height):
                    ...
                    window.set_cursor_real_position(
                        2, window.cursor_real_pos[1])
                else:
                    window.set_cursor_position(1, window.cursor[1]+1)
                    window.set_cursor_real_position(
                        2, window.cursor_real_pos[1]+self.linesize)
            else:
                rect = QRectF(window.cursor_real_pos[0], window.cursor_real_pos[1], self.pbuffer[window.id].width(
                )-window.cursor_real_pos[0], self.linesize)
    ...
    def clean_input_buffer_from_screen(self):
        ...
        rect.setWidth(self.pbuffer[0].width() -
                      self.lastwindow.cursor_real_pos[0]+1)
        rect.setHeight(self.linesize)
        txtbuffer = ''
        ...
        if ((w.id >= 0) and (w.id < 8)):
            ...
            if w.line_count > 0:
                self.pbuffer_painter[w.id].eraseRect(
                    QRectF(0, 0, self.pbuffer[w.id].width(), w.line_count*self.linesize))
            else:
                self.pbuffer_painter[w.id].eraseRect(QRectF(0, 0, self.pbuffer[w.id].width(
                ), 24*self.linesize))  # TODO: Fix hardcoded linecount
    ...
    def split_window(self, lines, ver):
        ...
        if (lines == 0):  # Unsplit
            ...
            if (self.pbuffer[1] != None):  # Window needs resizing
                ...
                self.pbuffer[1] = QImage(self.pbuffer[0].width(
                ), lines*self.linesize, QImage.Format_RGB32)
